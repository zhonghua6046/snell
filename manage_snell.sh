...
install_snell(){
    ...
    ARCH=$(uname -m)
    case $ARCH in
        x86_64) ARCH_URL="https://dl.nssurge.com/snell/snell-server-v5.0.0-linux-amd64.zip";;
        i386 | i686) ARCH_URL="https://dl.nssurge.com/snell/snell-server-v5.0.0-linux-i386.zip";;
        aarch64) ARCH_URL="https://dl.nssurge.com/snell/snell-server-v5.0.0-linux-aarch64.zip";;
        armv7l) ARCH_URL="https://dl.nssurge.com/snell/snell-server-v5.0.0-linux-armv7l.zip";;
        *) echo -e "${Error} 不支持的服务器架构: $ARCH"; exit 1;;
    esac
    echo -e "${Info} 检测到架构为 ${ARCH}, 开始下载..."
    wget -O snell-server.zip "$ARCH_URL"
    unzip -o snell-server.zip -d /usr/local/bin/
    chmod +x "$SNELL_BIN_FILE"
    rm snell-server.zip
    echo -e "${Info} Snell Server 程序安装完成。"
...
uninstall_snell(){
    ...
    rm -f "$SNELL_SERVICE_FILE" "$SNELL_BIN_FILE"
...
main_menu(){
    ...
    echo ""
    echo "================================================"
    read -p "请输入您的选择 [0-4]: " user_choice
