...
# =========================================================
# Realm 管理脚本 (v3.4 智能识别版)
# 核心升级：
# 1. 【智能监听】自动检测本机是否有 IPv6 地址
#    - 有 IPv6 -> 监听 [::] (双栈/v6机器)
#    - 无 IPv6 -> 监听 0.0.0.0 (大陆/v4机器)
# 2. 继承之前所有的修复 (防卡死、路径修复、自动纠错)
# =========================================================
...
# --- 核心功能：自动获取最佳监听 IP ---
get_auto_listen_ip() {
    # 方法：检查是否有全局范围(scope global)的 IPv6 地址
    # 排除掉 lo (本地环回) 和 link (链路本地 fe80)
    local has_ipv6=$(ip -6 addr show scope global 2>/dev/null)
    ...
    if [[ "$ipv6_disabled" -eq 1 ]]; then
        ...
        # 有公网 IPv6 地址 -> 使用 [::] (通常兼容双栈)
        echo "[::]"
    else
        # 没有公网 IPv6 -> 为了稳妥，使用 0.0.0.0
        echo "0.0.0.0"
    fi
}
...
install_realm() {
    ...
    if [[ -f "$INSTALL_DIR" ]]; then rm -f "$INSTALL_DIR"; fi
...
Restart=on-failure
RestartSec=5s
ExecStartPre=$BIN_FILE --version
ExecStart=$BIN_FILE -c realm.toml
...
add_rule() {
    ...
    # === 调用检测函数，获取最佳监听IP ===
    local listen_ip=$(get_auto_listen_ip)

    while true; do
        echo -e "\n${BLUE}=== 添加转发规则 ===${PLAIN}"
        echo -e "本机监听 IP 将自动设为: ${GREEN}$listen_ip${PLAIN} (自动检测)"
        
        echo -e "请输入本地监听端口 ${YELLOW}(例如 20000)${PLAIN}:"
        read -r lport
        [[ -z "$lport" ]] && break 
        
        echo -e "请输入目标 IP/域名 ${YELLOW}(支持 DDNS)${PLAIN}:"
        read -r raddr
...
[[endpoints]]
listen = "$listen_ip:$lport"
remote = "$raddr:$rport"
EOF
        success "规则已添加: $listen_ip:$lport -> $raddr:$rport"
...
show_menu() {
    while true; do
        ...
        echo -e "       Realm 管理脚本                       "
        echo -e "==========================================="
        echo -e "  1. 安装 Realm (含自检)"
        echo -e "  2. 更新 Realm"
        echo -e "  3. 卸载 Realm"
        echo -e "-------------------------------------------"
        echo -e "  4. 添加转发规则 (自动适配 IPv4/IPv6)"
        echo -e "  5. 修改配置文件"
        echo -e "  6. 查看运行日志"
        echo -e "-------------------------------------------"
        echo -e "  7. 启动服务"
